...
struct GoogleVideo: Codable {
    let videos: [GVideo]
}

class GoogleVideoService: VideoService {
    typealias ModelType = VideoData
    typealias CodableType = GVideo
    private var videos: [GVideo] = []
    private var mappedVideos: [VideoData] = []
    private let jsonFileName = "google-video-data.json"
    ...
    private func loadVideosFromJSON() -> AnyPublisher<[GVideo], Error> {
        return Future<[GVideo], Error> { promise in
            DispatchQueue.global().async {
                do {
                    // Read the JSON file
                    let data = try Data(contentsOf: Bundle.main.url(forResource: self.jsonFileName, withExtension: nil)!)
                    let videoData = try JSONDecoder().decode(GoogleVideo.self, from: data)
    ...
    private func createFallbackVideos() -> [GVideo] {
        return [
            GVideo(id: "v1",
